...
export function buildClient(mnemonic: string, ddk: DdkLib): Client {
  ...
  const walletProvider = new BitcoinJsWalletProvider({
    network: NETWORK,
    mnemonic,
  ...
  // which matters with a room of attendees hitting mempool.space at once.
  walletProvider.getUnusedAddress = async (change = false) =>
    (await walletProvider.getAddresses(0, 1, change))[0];
  client.addProvider(walletProvider as never);
